...
public class IPFS {
    ...
    public IPFS(String host, int port, String version) {
        ...
        try {
            ...
            if (detected.isBefore(MIN_VERSION)) throw new IllegalStateException("You need to use a more recent version of IPFS! >= " + MIN_VERSION);
        } catch (IOException e) {
    ...
    public List<MerkleNode> ls(Multihash hash) throws IOException {
        ...
        List<Object> list = (List<Object>) res.get("Objects");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return list.stream().map(x -> MerkleNode.fromJSON((Map) x)).collect(Collectors.toList());
        } else {
            List<MerkleNode> merkleNodeList = new ArrayList<>();
            for (Object o : list) {
                merkleNodeList.add(MerkleNode.fromJSON((Map) o));
            }
    ...
    public List<MerkleNode> add(List<NamedStreamable> files, boolean wrap, boolean hashOnly) throws IOException {
        ...
        for (NamedStreamable file : files) {
            if (file.isDirectory()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    m.addSubtree(file.getName().get(), file);
                }
            } else {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        ;
        String res = m.finish();
        List<Object> list = JSONParser.parseStream(res);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return list.stream().map(x -> MerkleNode.fromJSON((Map<String, Object>) x)).collect(Collectors.toList());
        } else {
            List<MerkleNode> merkleNodeList = new ArrayList<>();
            for (Object o : list) {
                merkleNodeList.add(MerkleNode.fromJSON(o));
            }
            return merkleNodeList;
        }
    }
    ...
    public List<Multihash> refs(Multihash hash, boolean recursive) throws IOException {
        ...
        List<Object> list = JSONParser.parseStream(jsonStream);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return list.stream().map(m -> (String) (((Map) m).get("Ref"))).map(Cid::decode).collect(Collectors.toList());
        } else {
            List<Multihash> multihashList = new ArrayList<>();
            for (Object o : list) {
                multihashList.add(Cid.decode((String) (((Map) o).get("Ref"))));
            }
    ...
    public Map mount(java.io.File ipfsRoot, java.io.File ipnsRoot) throws IOException {
        ...
        return (Map) retrieveAndParse(
                "mount?arg=" + (ipfsRoot != null ? ipfsRoot.getPath() : "/ipfs") + "&arg=" + (ipnsRoot != null ? ipnsRoot.getPath() : "/ipns"));
    }
    ...
    public class Pin {
        public List<Multihash> add(Multihash hash) throws IOException {
            List<Object> list = ((List<Object>) ((Map) retrieveAndParse("pin/add?stream-channels=true&arg=" + hash)).get("Pins"));
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return list.stream().map(x -> Cid.decode((String) x)).collect(Collectors.toList());
            } else {
                List<Multihash> multihashList = new ArrayList<>();
                for (Object o : list) {
                    multihashList.add(Cid.decode((String) o));
                }
                return multihashList;
            }
        }
        ...
        public Map<Multihash, Object> ls(PinType type) throws IOException {
            Set<Map.Entry<String, Object>> set =
                    ((Map<String, Object>) (((Map) retrieveAndParse("pin/ls?stream-channels=true&t=" + type.name())).get("Keys"))).entrySet();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return set.stream().collect(Collectors.toMap(x -> Cid.decode(x.getKey()), x -> x.getValue()));
            } else {
                Map<Multihash, Object> multihashObjectMap = new HashMap<>();
                for (Map.Entry<String, Object> map : set) {
                    multihashObjectMap.put(Cid.decode(map.getKey()), map.getValue());
                }
                return multihashObjectMap;
            }
        }
        ...
        public List<Multihash> rm(Multihash hash, boolean recursive) throws IOException {
            ...
            List<Object> list = (List<Object>) json.get("Pins");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return list.stream().map(x -> Cid.decode((String) x)).collect(Collectors.toList());
            } else {
                List<Multihash> multihashList = new ArrayList<>();
        ...
        public List<MultiAddress> update(Multihash existing, Multihash modified, boolean unpin) throws IOException {
            List<Object> list = (List<Object>) ((Map) retrieveAndParse(
                    "pin/update?stream-channels=true&arg=" + existing + "&arg=" + modified + "&unpin=" + unpin)).get("Pins");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return list.stream().map(x -> new MultiAddress((String) x)).collect(Collectors.toList());
            } else {
                List<MultiAddress> multiAddressList = new ArrayList<>();
                for (Object o : list) {
                    multiAddressList.add(new MultiAddress((String) o));
                }
    ...
    public class Key {
        public KeyInfo gen(String name, Optional<String> type, Optional<String> size) throws IOException {
            return KeyInfo.fromJson(
                    retrieveAndParse("key/gen?arg=" + name + type.map(t -> "&type=" + t).orElse("") + size.map(s -> "&size=" + s).orElse("")));
        }

        public List<KeyInfo> list() throws IOException {
            List<Object> list = (List<Object>) ((Map) retrieveAndParse("key/list")).get("Keys");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return list.stream().map(KeyInfo::fromJson).collect(Collectors.toList());
            } else {
                List<KeyInfo> keyInfoList = new ArrayList<>();
                for (Object o : list) {
                    keyInfoList.add(KeyInfo.fromJson(o));
                }
                return keyInfoList;
            }
        }
        ...
        public List<KeyInfo> rm(String name) throws IOException {
            List<Object> list = (List<Object>) ((Map) retrieveAndParse("key/rm?arg=" + name)).get("Keys");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                return list.stream().map(KeyInfo::fromJson).collect(Collectors.toList());
            } else {
                List<KeyInfo> keyInfoList = new ArrayList<>();
    ...
    public class Block {
        ...
        public List<MerkleNode> put(List<byte[]> data, Optional<String> format) throws IOException {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                List<MerkleNode> merkleNodeList = new ArrayList<>();
                for (byte[] b : data) {
                    merkleNodeList.add(put(b, format));
                }
                return merkleNodeList;
            }
        }
        ...
        public MerkleNode put(byte[] data, Optional<String> format) {
            ...
            List<Object> list = JSONParser.parseStream(res);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                return list.stream().map(x -> MerkleNode.fromJSON((Map<String, Object>) x)).findFirst().get();
            } else {
                return MerkleNode.fromJSON(list.get(0));
            }
        }
    ...
    public class IPFSObject {
        ...
        public List<MerkleNode> put(String encoding, List<byte[]> data) throws IOException {
            if (!"json".equals(encoding) && !"protobuf".equals(encoding)) throw new IllegalArgumentException("Encoding must be json or protobuf");
        ...
        public MerkleNode patch(Multihash base, String command, Optional<byte[]> data, Optional<String> name, Optional<Multihash> target)
                throws IOException {
            if (!ObjectPatchTypes.contains(command)) throw new IllegalStateException("Illegal Object.patch command type: " + command);
            ...
            switch (command) {
                case "add-link":
                    if (!target.isPresent()) throw new IllegalStateException("add-link requires name and target!");
                case "rm-link":
                ...
                case "append-data":
                    ...
                    Multipart m = new Multipart(
                            "http://" + host + ":" + port + version + "object/patch/" + command + "?arg=" + base.toBase58() + "&stream-channels=true",
                            "UTF-8");
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public List<MultiAddress> bootstrap() throws IOException {
        List<String> list = (List<String>) retrieveMap("bootstrap/").get("Peers");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            List<MultiAddress> multiAddressList = new ArrayList<>();
            for (String s : list) {
                multiAddressList.add(new MultiAddress(s));
            }
            return multiAddressList;
        }
    }
    ...
    public class Bootstrap {
        ...
        public List<MultiAddress> add(MultiAddress addr) throws IOException {
            List<String> list = (List<String>) retrieveMap("bootstrap/add?arg=" + addr).get("Peers");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                return list.stream().map(x -> new MultiAddress(x)).collect(Collectors.toList());
            } else {
                List<MultiAddress> multiAddressList = new ArrayList<>();
                for (String s : list) {
                    multiAddressList.add(new MultiAddress(s));
        ...
        public List<MultiAddress> rm(MultiAddress addr, boolean all) throws IOException {
            List<String> list = (List<String>) retrieveMap("bootstrap/rm?" + (all ? "all=true&" : "") + "arg=" + addr).get("Peers");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
    ...
    public class Swarm {
        public List<Peer> peers() throws IOException {
            ...
            List<Object> list = (List<Object>) m.get("Peers");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                return list.stream().flatMap(json -> {
                    try {
