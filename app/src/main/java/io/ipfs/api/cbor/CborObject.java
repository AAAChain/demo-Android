...
public interface CborObject {
    ...
    final class CborMap implements CborObject {
        ...
        public static CborMap build(Map<String, CborObject> values) {
            Set<Map.Entry<String, CborObject>> set = values.entrySet();
            SortedMap<CborObject, CborObject> transformed;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                transformed = set.stream().collect(Collectors.toMap(e -> new CborString(e.getKey()), e -> e.getValue(), (a, b) -> a, TreeMap::new));
            } else {
                transformed = new TreeMap<>();
                for (Map.Entry<String,CborObject> entry:set){
                    transformed.put(new CborString(entry.getKey()),entry.getValue());
                }
            }
            return new CborMap(transformed);
        }

        @Override public void serialize(CborEncoder encoder) {
            try {
    ...
    final class CborByteArray implements CborObject, Comparable<CborByteArray> {
        ...
        @Override public int compareTo(CborByteArray other) {
    ...
    final class CborString implements CborObject, Comparable<CborString> {
        ...
        @Override public int compareTo(CborString cborString) {
    ...
    final class CborLong implements CborObject, Comparable<CborLong> {
        ...
        @Override public int compareTo(CborLong other) {
    ...
    final class CborNull implements CborObject, Comparable<CborNull> {
        ...
        @Override public int compareTo(CborNull cborNull) {
            return 0;
        }
